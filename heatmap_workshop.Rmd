...
dist(M)
```
This tells us how similar (or distanct) genes are. If we want to know how similar samples are, we can take the transpose of the matrix:
```{r}
dist(t(M))
```
...
You may want to use gene names instead of gene identifiers in your heatmap.
