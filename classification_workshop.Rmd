...
Split your data into a training set and test set. You'll have to decide how many samples to use for each. 

Use your test set to build a model with `naiveBayes()`.

#### Test

Use your model with R's `predict()` function to predict the labels on the training data.

Do the same for the test data.

Plot an ROC curve of each set of predictions above. As in the toy data section, you'll need to use the `type='raw'` parameter to the `predict()` function.

#### Advanced

Try a different geneset, does performance change?

You can split your data into training, validation and test if you'd like to choose the geneset based on validation performance. You can then get real performance metrics for the resulting classifier using the test set.
